...
class Links_Action extends Typecho_Widget implements Widget_Interface_Do
{
	...
	public function insertLink()
	{
		...
		/** 取出数据 */
		$link = $this->request->from('name', 'url', 'sort', 'image', 'description', 'username');
		$link['sort_order'] = $this->db->fetchObject($this->db->select(array('MAX(sort_order)' => 'maxOrder'))->from($this->prefix.'links'))->maxOrder + 1;
	...
	public function updateLink()
	{
		...
		/** 取出数据 */
		$link = $this->request->from('lid', 'name', 'sort', 'image', 'url', 'description', 'username');
    ...
    public function sortLink()
    {
        ...
        if ($links && is_array($links)) {
			foreach ($links as $sort => $lid) {
				$this->db->query($this->db->update($this->prefix.'links')->rows(array('sort_order' => $sort + 1))->where('lid = ?', $lid));
			}
        }
    }
