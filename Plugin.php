...
class Links_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function linksInstall()
	{
		...
		try {
			...
			if(('Mysql' == $type && 1050 == $code) ||
				...
				try {
					$script = 'SELECT `lid`, `name`, `url`, `sort`, `image`, `description`, `username`, `sort_order` from `' . $prefix . 'links`';
	...
	public static function form($action = NULL)
	{
		...
		/** 自定义数据 */
		$username = new Typecho_Widget_Helper_Form_Element_Text('username', NULL, NULL, _t('自定义数据'), _t('该项用于用户自定义数据扩展'));
	...
	public static function output_str($pattern=NULL, $links_num=0, $sort=NULL)
	{
		...
		$sql = $sql->order($prefix.'links.sort_order', Typecho_Db::SORT_ASC);
		...
		foreach ($links as $link) {
			...
			$str .= str_replace(
				array('{lid}', '{name}', '{url}', '{sort}', '{title}', '{description}', '{image}', '{username}'),
				array($link['lid'], $link['name'], $link['url'], $link['sort'], $link['description'], $link['description'], $link['image'], $link['username']),
				$pattern
			);
		}
		return $str;
